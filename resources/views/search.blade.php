...
@section('title', 'ReFilm | Search Film')
...
<main class="px-4 lg:px-10 grid grid-cols-1 lg:grid-cols-4 gap-4 mt-[140px]">
    <div class="lg:col-span-4 flex items-center justify-between">
        <h1 class="text-white text-2xl font-bold">Search results '{{$search}}'</h1>
            ...
            @if ($films->hasPages())
                <div class="flex gap-2 mt-4">
                    @if (!$films->onFirstPage())
                        <a href="{{ $films->appends(['search' => $search])->previousPageUrl() }}" class="bg-[#22252F] text-white px-4 py-2 rounded-lg cursor-pointer bg-gradient-to-r from-[#FC882F] to-[#2EBCF9]">Previous</a>
                    @endif
                    @if ($films->hasMorePages())
                        <a href="{{ $films->appends(['search' => $search])->nextPageUrl() }}" class="bg-[#22252F] text-white px-4 py-2 rounded-lg cursor-pointer bg-gradient-to-r from-[#FC882F] to-[#2EBCF9]">Next</a>
                    @endif
                </div>
            @endif
        </div>

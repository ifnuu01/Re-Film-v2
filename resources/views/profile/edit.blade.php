...
@section('title', 'ReFilm | Edit Profile')

@section('content')
<main class="px-4 lg:px-10 mt-[130px]">
        @if (session('status'))
            <div class="bg-green-500 text-white p-4 rounded-lg mb-4">
                {{ session('status') }}
            </div>
        @endif
        <h3 class="text-2xl text-white font-bold">Settings</h3>
        <section class="flex flex-col items-start gap-x-4 lg:flex-row lg:items-center w-full bg-[#22252F] p-4 rounded-lg mt-4">
            <div class="flex items-center gap-x-4">
                ...
                @endif
                <div>
                    <h2 class="text-white text-2xl font-bold">{{ Str::limit(auth()->user()->name, 15) }}</h2>
                    <p class="text-gray-400">{{ Str::limit(auth()->user()->email,20) }}</p>
                </div>
            </div>
            <form action="{{route('profile.photo.update')}}" method="post" enctype="multipart/form-data" class="flex mt-4 lg:mt-0 lg:flex-row gap-x-4 items-center ml-auto gap-y-2">
                @csrf
                @method('PATCH')
                <input type="file" name="photo" id="photo" class="hiddenc">
        ...
        <section class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div class="bg-[#22252F] p-4 rounded-lg mt-4">
